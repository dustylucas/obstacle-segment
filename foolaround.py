...
if __name__ == '__main__':
    ...
    while True:
        ...
        h = 1000  # height in cm
        pitch = np.radians(45)  # convert pitch to radians
        sensor_size = (36, 24)  # sensor dimensions in mm (common full frame size)
        image_size = src.shape  # image dimensions in pixels

        # Compute field of view
        fov_horizontal = 2 * np.arctan((sensor_size[0] / 2) / f)
        fov_vertical = 2 * np.arctan((sensor_size[1] / 2) / f)

        # Compute ground distances
        distance_horizontal = h * np.tan(pitch + fov_horizontal / 2)
        distance_vertical = h * np.tan(pitch + fov_vertical / 2)

        # Image points (corners of the image)
        ...
        # Ground points calculated by projecting image points
